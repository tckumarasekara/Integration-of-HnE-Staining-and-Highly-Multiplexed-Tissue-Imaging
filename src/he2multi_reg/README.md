...
Transform segmentation masks based on the transformation maps produced with the command `register`. Currently compatible with feature based registration transformations, but the python library is compatible with both feature and intensity based registration transformations.

```bash
he2multi-reg transform-seg-mask <mask_path> <fixed_path> <output_folder_path> <tform_map_path> <moving_px_sz> [--fixed-px-sz]
```
